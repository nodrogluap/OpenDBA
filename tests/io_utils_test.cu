...
TEST_CASE( " Write Slow5 Output " ){
	...
	SECTION("Good File Data"){
		...
		int result = writeSlow5Output(stringToChar(series_filename_seq), stringToChar(slow5_output), sequence_names, sequences, sequence_lengths, num_sequences);
		...
		int num_result_sequences = read_slow5_data(stringToChar(slow5_output), result_sequences, result_sequence_names, result_sequence_lengths);
	...
	SECTION("Wrong Size File Data"){
		...
		std::cerr << "------TEST 2: enforce same length on SLOW5 output ------" << std::endl;
